import os, sqlite3, secrets, string, calendar, hashlib, hmac, time
...
def setup():
    c=conn(); q=c.cursor()
    q.execute("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY,username TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,salt TEXT NOT NULL,created_at TEXT NOT NULL,last_login_at TEXT,last_seen_at TEXT,disabled INTEGER NOT NULL DEFAULT 0)")
    q.execute("CREATE TABLE IF NOT EXISTS sessions(token_hash TEXT PRIMARY KEY,username TEXT NOT NULL,created_at TEXT NOT NULL,expires_at TEXT NOT NULL,last_seen_at TEXT NOT NULL)")
    q.execute("CREATE TABLE IF NOT EXISTS licenses(license_key TEXT PRIMARY KEY,duration_type TEXT NOT NULL,duration_amount INTEGER NOT NULL,activated INTEGER NOT NULL DEFAULT 0,bound_hwid TEXT,activated_at TEXT,expires_at TEXT,created_at TEXT NOT NULL,revoked INTEGER NOT NULL DEFAULT 0,owner_username TEXT)")
    q.execute("CREATE TABLE IF NOT EXISTS audit_logs(id INTEGER PRIMARY KEY,created_at TEXT NOT NULL,action TEXT NOT NULL,username TEXT,license_key TEXT,details TEXT)")
    q.execute("CREATE TABLE IF NOT EXISTS settings(k TEXT PRIMARY KEY,v TEXT NOT NULL)")
    for table,col,definition in [
        ("users","disabled","INTEGER NOT NULL DEFAULT 0"),("users","last_login_at","TEXT"),("users","last_seen_at","TEXT"),
        ("sessions","last_seen_at","TEXT"),("licenses","owner_username","TEXT"),("licenses","bound_hwid","TEXT"),
        ("licenses","activated_at","TEXT"),("licenses","expires_at","TEXT"),("licenses","revoked","INTEGER NOT NULL DEFAULT 0")]:
        cols={r["name"] for r in q.execute(f"PRAGMA table_info({table})")}
        if col not in cols: q.execute(f"ALTER TABLE {table} ADD COLUMN {col} {definition}")
    q.execute("INSERT OR IGNORE INTO settings(k,v) VALUES('maintenance','0')")
    c.commit(); c.close()
setup()
...
def audit(action,username=None,key=None,details=""):
    c=conn(); c.execute("INSERT INTO audit_logs(created_at,action,username,license_key,details) VALUES(?,?,?,?,?)",(iso(now()),action,username,key,str(details)[:500])); c.commit(); c.close()
...
def new_session(user):
    ...
    c=conn(); c.execute("INSERT INTO sessions VALUES(?,?,?,?,?)",(th(t),user,iso(n),iso(e),iso(n))); c.commit(); c.close(); return t
...
def user_from_auth(auth):
    ...
    c=conn(); r=c.execute("SELECT * FROM sessions WHERE token_hash=?",(th(t),)).fetchone()
    if not r: c.close(); raise HTTPException(401,"Session expired. Please sign in again.")
    if now() >= dt(r["expires_at"]):
        c.execute("DELETE FROM sessions WHERE token_hash=?",(th(t),)); c.commit(); c.close()
        raise HTTPException(401,"Session expired. Please sign in again.")
    u=c.execute("SELECT disabled FROM users WHERE username=?",(r["username"],)).fetchone()
    if not u or u["disabled"]: c.close(); raise HTTPException(403,"Account disabled.")
    seen=iso(now()); c.execute("UPDATE sessions SET last_seen_at=? WHERE token_hash=?",(seen,th(t))); c.execute("UPDATE users SET last_seen_at=? WHERE username=?",(seen,r["username"])); c.commit(); c.close()
    return r["username"]
...
def register(request:Request,b:Auth):
    ...
    try: c.execute("INSERT INTO users(username,password_hash,salt,created_at) VALUES(?,?,?,?)",(u,h,s,iso(now()))); c.commit()
...
def login(request:Request,b:Auth):
    ...
    if maintenance(): raise HTTPException(503,"The service is currently in maintenance mode.")
    c=conn(); r=c.execute("SELECT * FROM users WHERE username=?",(b.username.strip(),)).fetchone(); c.close()
...
def activate(request:Request,b:LicenseReq,auth:str|None=Header(default=None, alias="Authorization")):
    ...
    if not r["activated"]:
        ...
        c.execute("UPDATE licenses SET activated=1,bound_hwid=?,activated_at=?,expires_at=?,owner_username=? WHERE license_key=?",(b.hwid,iso(a),iso(e),u,key)); c.commit(); c.close()
...
def admin_generate(b:Gen,x: str|None=Header(default=None,alias="X-Admin-Secret")):
    ...
    while True:
        k=gen_key()
        if not c.execute("SELECT 1 FROM licenses WHERE license_key=?",(k,)).fetchone(): break
    c.execute("INSERT INTO licenses(license_key,duration_type,duration_amount,created_at) VALUES(?,?,?,?)",(k,b.duration_type.lower(),b.duration_amount,iso(now()))); c.commit(); c.close()
...
def extend(key:str,b:Extend,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); r=c.execute("SELECT * FROM licenses WHERE license_key=?",(key.upper(),)).fetchone()
    ...
    c.execute("UPDATE licenses SET expires_at=?,activated=1 WHERE license_key=?",(iso(e),key.upper())); c.commit(); c.close()
...
def revoke(b:KeyReq,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); r=c.execute("SELECT owner_username FROM licenses WHERE license_key=?",(b.license_key.upper(),)).fetchone(); cur=c.execute("UPDATE licenses SET revoked=1 WHERE license_key=?",(b.license_key.upper(),)); c.commit(); c.close()
    if cur.rowcount==0: raise HTTPException(404,"License not found.")
    if r and r["owner_username"]:
        c=conn(); c.execute("DELETE FROM sessions WHERE username=?",(r["owner_username"],)); c.commit(); c.close()
...
def unrevoke(b:KeyReq,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); cur=c.execute("UPDATE licenses SET revoked=0 WHERE license_key=?",(b.license_key.upper(),)); c.commit(); c.close()
    if cur.rowcount==0: raise HTTPException(404,"License not found.")
...
def reset_hwid(b:KeyReq,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); r=c.execute("SELECT owner_username FROM licenses WHERE license_key=?",(b.license_key.upper(),)).fetchone(); cur=c.execute("UPDATE licenses SET activated=0,bound_hwid=NULL,activated_at=NULL,expires_at=NULL,owner_username=NULL WHERE license_key=?",(b.license_key.upper(),)); c.commit(); c.close()
    if cur.rowcount==0: raise HTTPException(404,"License not found.")
    if r and r["owner_username"]:
        c=conn(); c.execute("DELETE FROM sessions WHERE username=?",(r["owner_username"],)); c.commit(); c.close()
...
def disable(username:str,b:Toggle,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); cur=c.execute("UPDATE users SET disabled=? WHERE username=?",(int(b.disabled),username)); c.commit(); c.close()
    if cur.rowcount==0: raise HTTPException(404,"User not found.")
    if b.disabled:
        c=conn(); c.execute("DELETE FROM sessions WHERE username=?",(username,)); c.commit(); c.close()
...
def reset_password(username:str,b:ResetPW,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    ...
    h,s=pw_hash(b.new_password); c=conn(); cur=c.execute("UPDATE users SET password_hash=?,salt=? WHERE username=?",(h,s,username)); c.commit(); c.close()
    if cur.rowcount==0: raise HTTPException(404,"User not found.")
    c=conn(); c.execute("DELETE FROM sessions WHERE username=?",(username,)); c.commit(); c.close(); audit("USER_PASSWORD_RESET",username); return {"success":True}
...
def force_logout(username:str,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); c.execute("DELETE FROM sessions WHERE username=?",(username,)); c.commit(); c.close(); audit("USER_FORCE_LOGOUT",username); return {"success":True}
...
def set_maintenance(b:Maint,x:str|None=Header(default=None,alias="X-Admin-Secret")):
    admin(x); c=conn(); c.execute("UPDATE settings SET v=? WHERE k='maintenance'",("1" if b.enabled else "0",)); c.commit(); c.close(); audit("MAINTENANCE_ON" if b.enabled else "MAINTENANCE_OFF"); return {"success":True,"maintenance":b.enabled}
